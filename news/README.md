...
首页右上角“小游戏”进入 `/games/`，提供打砖块（五关）、飞机大战（自动开火、敌机逐渐加速）和跳跃跑酷（二段跳）。三款均为本站原生 Canvas 实现，不加载第三方游戏、广告、字体或网络素材。

- 手机拖动控制挡板和飞机，点按跑酷画面跳跃；电脑可用方向键 / WASD，跑酷用空格或向上键跳跃，P / Escape 暂停。
...
规则测试随发布执行：`node --test tests/games-core.cjs`。本地构建并启动预览后，可运行触屏、暂停、成绩保存和离线打开检查：
