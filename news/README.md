...
首页右上角“小游戏”进入 `/games/`，提供打砖块（五关）、飞机大战（自动开火、敌机逐渐加速）、跳跃跑酷（二段跳）、叠高楼（对齐落块）和连色消除（消除相连同色块、全清奖励）。五款均为本站原生 Canvas 实现，不加载第三方游戏、广告、字体或网络素材。

- 游戏页使用独立满屏布局，游戏画面随可见视口和安全区域适配，横竖屏保持物体比例；说明收进“玩法”。尺寸变化时暂停并调整当前局的位置，不清空成绩。
